...
const COURSES = {
  ...
  COURSE_0_FOUNDATION: {
    ...
    lessons: [
      ...
      { id:"F05",  name:"เทคนิค ASBC",                       dur:"20 นาที", url:"https://www.youtube.com/embed/QbuyU8EGMjU?si=npNmAGbOt4I0Almb", qg:"QG-04" },
  ...
  COURSE_1_SIGNAL: {
    ...
    lessons: [
      ...
      { id:"S06", name:"มาตรฐานสากลและบทสรุป",                       dur:"30 นาที", url:"https://www.youtube.com/embed/QbuyU8EGMjU?si=npNmAGbOt4I0Almb", qg:"QG-08" },
    ],
  ...
  COURSE_3_BRAND_HOST: {
    ...
    lessons: [
      { id:"BH1", name:"5 Hidden Souls — Host Archetype DNA",              dur:"วัน 1 · 09:00-10:30", url:null, qg:"QG-09", sessionCode:true, day:1 },
...
// จริง — ชื่อเต็มราย QG อยู่ในตาราง Supabase `quiz_groups` ต้องตรงกับ
// src/lib/ppact.ts ของเว็บหลักเสมอ
// ============================================================
const RADAR_DIMS = [
...
// ไม่สำเร็จ (ดู quizBank.js). QG-08/09/10 มีข้อจริงใน Supabase แล้ว
// (2569-09-23) แต่ไม่มีในชุด fallback นี้ — ถ้าโหลดไม่สำเร็จ บทของ
// QG เหล่านั้นจะผ่านทาง completeLessonWithoutQuiz แทน ดู Bible ภาคผนวก
// Z2.1 สำหรับแผนเดิมที่ตั้งใจไว้ 116 ข้อ
// ============================================================
...
// The live question set — starts out pointing at the bundled fallback,
// reassigned in place once loadQuizBank() resolves (see the App-level
// useEffect below). Every function in this file reads QUIZ_BANK fresh on
// each call, so nothing needs to be re-rendered for the swap to take
// effect — it just has to happen before the learner opens a quiz.
let QUIZ_BANK = QUIZ_BANK_FALLBACK;
...
function buildRadar(lessonScores) {
  // lessonScores: { [lessonId]: { qg, pct } } — pct is null for a lesson that
  // had no gradable questions (see completeLessonWithoutQuiz).
  //
  // Only QGs the learner actually has a score for are averaged. Padding every
  // missing QG with a 0 used to drag dimensions that span several QGs (Trust =
  // QG-06/08/09/10) down to ≤25–50% for every course, since no course covers
  // all four — a number that described the course layout, not the learner.
  const qgScores = {};
  ...
  return RADAR_DIMS.map(d => {
    const vals = d.qgs.flatMap(qg => qgScores[qg] || []);
...
function getRecommendations(lessonScores, enrolledCourses = []) {
  ...
  Object.values(lessonScores).forEach(({ qg, pct }) => {
    if (typeof pct !== "number") return;
    if (!(qg in qgScores) || pct < qgScores[qg]) qgScores[qg] = pct;
  });
  const recs = new Map();
    ...
    enrolledCourseIds.forEach(courseId => {
      ...
      course.lessons.forEach(lesson => {
        ...
        if (entry?.status === "completed") {
          ...
          scoresForCourse[lesson.id] = { qg: lesson.qg, pct: typeof entry.score === "number" ? entry.score : null };
          doneCount++;
        }
      });
      ...
      <div style={S.card}>
        ...
        {Object.entries(lessonScores).map(([lid, { pct, qg }]) => (
          <div key={lid} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
            ...
            <div style={S.barBg}><div style={S.barFill(pct ?? 0)} /></div>
            {typeof pct === "number"
              ? <div style={{ width:34, fontSize:12, color: pct>=70?"#1A6B3A":"#C0392B", textAlign:"right", fontWeight:600 }}>{pct}%</div>
              : <div style={{ width:34, fontSize:12, color:"#888", textAlign:"right" }}>—</div>}
          </div>
        ))}
...
export default function Creatr365LMS() {
  ...
  // Replace QUIZ_BANK with the real Supabase question set as early as
  // possible — before the learner can reach a pretest/quiz screen.
  // loadQuizBank() never throws; a null result (network failure, empty
  // table) just leaves QUIZ_BANK pointing at QUIZ_BANK_FALLBACK.
  useEffect(() => {
    let cancelled = false;
    loadQuizBank().then(questions => {
  ...
  function startLessonQuiz(lesson) {
    ...
    if (!qs.length) { completeLessonWithoutQuiz(lesson); return; }
  ...
  // A lesson whose QG has no gradable questions (bank not yet written, or a
  // failed load of an unseeded QG) must still be recorded server-side.
  // markLessonDone alone only reaches the legacy Apps Script mirror, so these
  // lessons used to relock on the next login and — because
  // issue_completion_record() requires every module_progress row to be
  // completed — silently block the course's Completion Record forever.
  // Saved with score null (not a fake 100) so it never inflates the radar.
  function completeLessonWithoutQuiz(lesson) {
    markLessonDone(lesson, null);
    apiSaveScore(student?.id, activeCourse, "no_quiz", lesson.qg, 0, 0, null, true, lesson.id);
    setScreen("course");
  }
